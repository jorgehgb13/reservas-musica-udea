  ...
  try {
    ...
    if (userId) {
      ...
      const active = (roomReservations || []).find(
        (r) => new Date(`${r.date}T${r.end_time}`) > now
      );
        ...
        return NextResponse.json(
          {
            ...
            message: `Ya tienes una reserva activa en ${active.rooms?.name || 'un espacio'} que termina el ${new Date(`${active.date}T${active.end_time}`).toLocaleString('es-CO')}. Podrás solicitar una nueva cuando termine.`,
          },
          { status: 403 }
        );
      }
