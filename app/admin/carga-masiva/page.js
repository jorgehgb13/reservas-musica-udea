  ...
  async function handleConfirmUpload() {
    ...
    for (const row of validRows) {
      try {
        ...
        // Se identifica la clase por espacio + nombre del curso (lo único que
        // no debería cambiar). Todo lo demás — docente, horario, días,
        // fecha de fin — se puede actualizar libremente sin crear duplicados.
        const { data: roomTemplates, error: findTemplateError } = await supabase
          .from('recurring_templates')
          .select('id, materia')
          .eq('room_id', row.room.id);
        if (findTemplateError) throw findTemplateError;

        const matchingTemplate = (roomTemplates || []).find(
          (t) => normalize(t.materia) === normalize(row.materia)
        );
        ...
        if (matchingTemplate) {
          ...
          // Se cancelan TODAS las reservas anteriores de esta clase, para
          // reemplazarlas por completo con el horario y fechas nuevas.
          const { data: oldReservations, error: findOldError } = await supabase
            .from('reservations')
            .select('id')
            .eq('recurring_template_id', templateId)
            .neq('status', 'cancelada')
            .neq('status', 'rechazada');
          if (findOldError) throw findOldError;
        ...
        const newRows = occurrences.map((date) => ({
          room_id: row.room.id,
          user_id: userId,
          date,
          start_time: row.horaInicio,
        ...
        if (newRows.length > 0) {
          ...
          if (!bulkInsertError) {
            ...
            // Si la creación en bloque falla (por ejemplo, un choque real de
            // horario con otra reserva de otra clase), se intenta fecha por
            // fecha para saber exactamente cuáles sí se pudieron crear.
        ...
        reservationsCancelled += cancelledOld;
        ...
        <div>
          <div style={{ background: '#E4F0EA', border: '1px solid #bcd9c9', borderRadius: 8, padding: 18, marginBottom: 20 }}>
            ...
            <p style={{ fontSize: 13, color: '#084F39', margin: 0 }}>
              {results.templatesCreated} materia(s) nueva(s) · {results.templatesUpdated} materia(s) ya existente(s) reemplazada(s) con los datos nuevos
              <br />
              {results.reservationsCreated} clase(s) reservada(s) creada(s) · {results.reservationsCancelled} reserva(s) anterior(es) cancelada(s) al reemplazar
              {results.reservationsSkipped > 0 && <> · {results.reservationsSkipped} omitida(s) por un choque real de horario con otra reserva</>}
            </p>
          </div>
                  ...
                  <tr key={r.rowNumber} style={{ borderBottom: '1px solid #DBDCCF' }}>
                    ...
                    <td style={{ padding: 6 }}>
                      {r.ok ? (
                        <>
                          {r.templateIsNew ? (
                            <span style={{ color: '#5B6B60' }}>materia nueva · </span>
                          ) : (
                            <span style={{ color: '#5B6B60' }}>
                              materia reemplazada ({r.cancelledOld} reserva(s) anterior(es) cancelada(s)) ·{' '}
                            </span>
                          )}
