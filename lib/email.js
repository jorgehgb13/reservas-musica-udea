// Envía correos reales usando la API de Resend. Requiere la variable de
// entorno RESEND_API_KEY (configurada en Vercel, nunca en el código).
...
const FROM_ADDRESS = 'Reservas Música UdeA <reservas@reservasmusicaudea.com>';

export async function sendEmail({ to, subject, html }) {
  if (!process.env.RESEND_API_KEY) {
    console.error('[email] Falta configurar RESEND_API_KEY en las variables de entorno.');
    return { ok: false };
  }
  ...
  try {
    ...
    if (!res.ok) {
      ...
      console.error('[email] Resend respondió con error:', res.status, errText);
      return { ok: false };
    }
